...
def main():
    ...
    test_video_dir = 'D:/The SquadSync/IEM-main_2/data/Videos'
    ...
    if len(TestDataset) == 0:
        logging.error("Validation dataset is empty. Exiting...")
        sys.exit(1)
    ...
    for epoch in range(args.epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter = 0
            # Save best model
            logging.info(f"New best validation loss: {best_val_loss} at epoch {epoch}")
            utils.save(model, os.path.join(model_path, 'best_weights.pt'))
        else:
            patience_counter += 1
            logging.info(f"No improvement in validation loss. Patience counter: {patience_counter}/{args.patience}")
